...
Run Snyk Code (SAST), Snyk Open Source (SCA), and Snyk Secrets against a
...
MODES:
  --staged       Pre-commit mode. Only files in the index ("git diff --cached")
                 ...
                 SCA findings are filtered to the staged manifests, secrets
                 findings are filtered to all staged files, and the footer
                 points at `git commit --no-verify` as the bypass.
...
def filter_secrets_findings(
    findings: List[Dict[str, Any]], staged: Optional[List[str]]
) -> List[Dict[str, Any]]:
    """Keep secrets findings whose file_path matches any staged file.

    Secrets can appear in any file type (code, config, dotfiles, etc.), so
    the filter uses the full staged-file list rather than just code files or
    manifests. Passing ``staged=None`` disables the filter — every finding is
    returned (full-repo mode).
    """
    if staged is None:
    ...
    if not staged:
        return []
    staged_norm = _staged_set(staged)
    return [f for f in findings if _vuln_path_matches(f.get("file_path", ""), staged_norm)]
...
def run_secrets_scan(workspace: Path, snyk_bin: str) -> Tuple[str, List[Dict[str, Any]]]:
    log("running snyk secrets test...")
    try:
        result = subprocess.run(
        ...
        log(f"snyk secrets test failed to launch: {e}")
        return "error", []
    if result.returncode > 1:
        status = _classify_snyk_failure(result.stderr, result.stdout)
        debug(f"snyk secrets test exit={result.returncode}, stderr={result.stderr[:300]}")
        return status, []
    return "success", parse_sast_results(result.stdout)
...
def _print_block_reason(
    ...
    secrets_vulns: Optional[List[Dict[str, Any]]] = None,
    secrets_fallback: str = "",
) -> None:
    ...
    # Render secrets findings using the same SAST group layout (SARIF output).
    _secrets = secrets_vulns or []
    secrets_groups: Dict[Tuple[str, int, int], List[Dict[str, Any]]] = {}
    for v in _secrets:
        key = (
            v.get("file_path", ""),
            v.get("start_line", 0),
        ...
        secrets_groups.setdefault(key, []).append(v)
    sorted_secrets_keys = sorted(
        secrets_groups.keys(),
        key=lambda k: (
            min(_SEVERITY_ORDER.get(v.get("severity", "low"), 4) for v in secrets_groups[k]),
            -len(secrets_groups[k]),
            k[0],
            k[1],
            k[2],
        ),
    )
    for key in sorted_secrets_keys:
        file_path, line, column = key
        print(
            _fmt_sast_group(file_path, line, column, secrets_groups[key], color),
            file=sys.stderr,
        )
    ...
    if secrets_fallback:
        print(secrets_fallback, file=sys.stderr)

    total = len(sast_vulns) + len(sca_vulns) + len(_secrets)
    if total:
        if staged_mode:
            print(
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.staged:
        ...
        if repo_root is None:
            repo_root = cwd
        # None signals "no filter" to filter_* functions.
        staged_code = None
    ...
    if args.staged:
        log(
            ...
            + f", {len(staged_all or [])} file(s) for secrets"
        )
    else:
    ...
    sast_vulns: List[Dict[str, Any]] = []
    sca_vulns: List[Dict[str, Any]] = []
    secrets_vulns: List[Dict[str, Any]] = []
    sast_fallback = ""
    sca_fallback = ""
    secrets_fallback = ""
    ...
    # Secrets can appear in any file, so run whenever any file is staged (or
    # in full-repo mode).
    should_run_secrets = staged_all is None or len(staged_all) > 0

    # SAST, SCA, and secrets shell out to independent CLI invocations, so we
    # fan them out across up to three threads. The threads sit blocked in
    # subprocess wait() — CPython releases the GIL there, so the processes run
    # in true parallel and the latency is max(SAST, SCA, secrets) rather than
    # their sum.
    with ThreadPoolExecutor(max_workers=3) as executor:
        ...
        secrets_future = (
            executor.submit(run_secrets_scan, repo_root, snyk_bin) if should_run_secrets else None
        )
        ...
        if secrets_future is not None:
            status, all_findings = secrets_future.result()
            if status == "success":
                secrets_vulns = filter_secrets_findings(all_findings, staged_all)
                log(f"Secrets: {len(secrets_vulns)} issue(s)")
            elif status == "auth_required":
                secrets_fallback = "Snyk CLI is not authenticated. Run `snyk auth` and re-run."
            else:
                secrets_fallback = (
                    "Snyk Secrets scan did not complete. Run `snyk secrets test` manually."
                )

    if not (
        sast_vulns or sca_vulns or secrets_vulns
        or sast_fallback or sca_fallback or secrets_fallback
    ):
    ...
    _print_block_reason(
        ...
        secrets_vulns=secrets_vulns,
        secrets_fallback=secrets_fallback,
    )
    return EXIT_BLOCK
